...
const getAllSongs = (req, res, next) => {
  ...
  db.any('SELECT songs.id, title, img_url, COUNT(favorites.song_id) AS favoritesCount, username, comments.comment_body AS comment, comments.song_id AS songComment, comments.user_id AS commenter FROM songs JOIN favorites ON songs.id = favorites.song_id JOIN users ON songs.user_id = users.id JOIN comments ON songs.id = comments.song_id GROUP BY songs.id, title, img_url, username, commenter ORDER BY songs.id DESC')
    .then(songs => {
      res.status(200).json({
        status: "success!",
        songs: songs,
